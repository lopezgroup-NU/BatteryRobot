...
def run_geis(output_file_name = "galvanostatic_eis", parameter_list = {}, save_to_db_folder = True, standard = False):
    ...
    #Parameters
    #------------------------------------------------------------------
    initial_freq = parameter_list.get("initial_freq", 100000) #250000
    ...
    while current_points < max_points:
        ...
        if not status:
            bad_reads += 1
            if bad_reads >= 30:
                print(f"skipping {freq:.3g} Hz after 30 failed reads")
                current_points += 1
                bad_reads = 0
            continue
        bad_reads = 0
...
def run_geis_cell(cell, output_file_name = "galvanostatic_eis",  parameter_list = {}, path_to_save_to = r"C:\AttomRobotFiles\Data\DB_Ciara\eis", save_to_db_folder = True, standard = False):
    ...
    try:
        pstat.open() # THIS command was never used before; I am not sure if this is something we should be using but it has to be opened in order to run pstat.close()
        mux.open()
        ...
        # pstat.close()
        # mux.close()
        # ADDED to release the pstat so we can run another test without it returning a task error
        # logic is that since the mux closes but not the pstat, the pstat is floating with the previous test case and won't accept another run
        try: pstat.set_cell(False)
...
if __name__ == "__main__":
    ...
    zcurve = run_geis(parameter_list = parameter_list)
